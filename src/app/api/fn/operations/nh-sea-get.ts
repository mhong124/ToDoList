...
export interface NhSeaGet$Params {
...
/**
 * Retrieve only the sea creature that are available in a specific month. Value may be the month's name (`jan`, `january`), the integer representing the month (`01`, `1`), or `current` for the current month. When `current` is specified, the return body will be an object with two arrays inside, one called `north` and the other `south` containing the sea creature available in each respective hemisphere. Note that the current month is calculated based off the API server's time, so it may be slightly off for you at the beginning or end of the month.
 */
  month?: string;
...
export function nhSeaGet(http: HttpClient, rootUrl: string, params: NhSeaGet$Params, context?: HttpContext): Observable<StrictHttpResponse<Array<NhSeaCreature>>> {
  const rb = new RequestBuilder(rootUrl, nhSeaGet.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<Array<NhSeaCreature>>;
    })
  );
}

nhSeaGet.PATH = '/nh/sea';
