...
export interface NhFishGet$Params {
...
/**
 * Retrieve only the fish that are available in a specific month. Value may be the month's name (`jan`, `january`), the integer representing the month (`01`, `1`), or `current` for the current month. When `current` is specified, the return body will be an object with two arrays inside, one called `north` and the other `south` containing the fish available in each respective hemisphere. Note that the current month is calculated based off the API server's time, so it may be slightly off for you at the beginning or end of the month.
 */
  month?: string;
...
export function nhFishGet(http: HttpClient, rootUrl: string, params: NhFishGet$Params, context?: HttpContext): Observable<StrictHttpResponse<Array<NhFish>>> {
  const rb = new RequestBuilder(rootUrl, nhFishGet.PATH, 'get');
  if (params) {
  ...
  return http.request(
    ...
    map((r: HttpResponse<any>) => {
      return r as StrictHttpResponse<Array<NhFish>>;
    })
  );
}

nhFishGet.PATH = '/nh/fish';
